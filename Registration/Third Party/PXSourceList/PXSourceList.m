...
- (BOOL)keyDownHelper:(NSInteger)row
{
    id item = [self itemAtRow:row];
    BOOL group = NO;
    if ([_secondaryDataSource respondsToSelector:@selector(sourceList:isUnselectable:)]) {
        group = [_secondaryDataSource sourceList:self isUnselectable:item];
    }
    return group||[self isGroupItem:item];
}
				...
				if([selectedIndexes count]==1&&!([theEvent modifierFlags] & NSShiftKeyMask))
				{
					...
					do {
						...
						if(newRow<0||newRow==[self numberOfRows])
							break;
					} while([self keyDownHelper:newRow]);
